...
- **Unbounded generic names** — `utils/`, `helpers/`, `lib/`, `common/`, `shared/`, `misc/`, `core/`. A smell when they grow without scope: anything can land there, and nothing can violate the name. A small, scoped `utils/` for genuinely cross-cutting helpers, or a repo-conventional `lib/` for shared reusable items, can be fine — the smell is gravity, not the letters.
- **Type-based folders without a bounded contract** — `interfaces/`, `types/`, `classes/`, `constants/`, `enums/`. A smell only when the folder becomes a default dumping ground and separates definitions from the behavior, schema, or public surface that gives them meaning. Legitimate when the folder is generated code, schema-only code, a published/public type surface, framework convention, or a small local bucket with a clear import contract.
...
The rule isn't "never use these names." The rule is: the name must give a useful prediction *in this project*. `utils/` for cross-cutting string formatting in a small repo is fine. `lib/` for shared reusable items is fine when that is the project's bounded convention. A broad folder as the default destination for anything that didn't fit elsewhere is the smell.
...
- **Sibling strategy test.** What is the grouping strategy for this folder's immediate children: concept, use case, layer, adapter role, lifecycle phase, framework convention, or something else? Do the siblings follow that strategy consistently? `auth/`, `billing/`, `orders/`, `utils/` may mix three concept folders with one unbounded junk drawer. Treat this as a medium-strength signal: a cross-cutting sibling can still belong here when the repo convention makes its shared role predictable, such as a bounded `lib/`, `schemas/`, `db/`, or `observability/`.
...
- **Prefer a clear shared-code convention.** "Used by auth, billing, and orders" is a prompt to identify the shared role, not an automatic rename. A bounded `lib/` or `shared/` folder can be a perfectly good repo convention. If the code has a more specific owner, consider that too: a domain concept (`money/`, `identity/`), support role (`db/`, `queue/`, `observability/`), or contract surface (`schemas/`, `contracts/`, `types/`). If the reuse is incidental, leaving it with the concept that owns the knowledge may be clearer than extracting it.
...
4. **What's left?** A truly cross-cutting helper (a string utility used by ten unrelated places) can stay in the project's bounded shared area (`lib/`, `shared/`, `utils/`) or move to a narrower name such as `string-formatting/` when that would improve predictiveness.
