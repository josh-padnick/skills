...
- **Sibling strategy test.** What is the grouping strategy for this folder's immediate children: concept, use case, layer, adapter role, lifecycle phase, framework convention, or something else? Do the siblings follow that strategy consistently? `auth/`, `billing/`, `orders/`, `utils/` mixes three concept folders with one unbounded junk drawer; the set is incoherent. A cross-cutting sibling can still belong here, but its name must state its actual contract: `money/`, `time/`, `observability/`, `db/`, or `schemas/` is checkable in a way `utils/` is not.
...
- **Put shared code under the smallest honest owner.** "Used by auth, billing, and orders" does not by itself justify `shared/` or `utils/`. If the code is a real domain concept, make it a named peer concept (`money/`, `identity/`). If it is infrastructure, make the support role explicit (`db/`, `queue/`, `observability/`). If it is a public or generated contract, keep it in a bounded surface (`schemas/`, `contracts/`, `types/`). If it is only coincidentally reused, leave it with the concept that owns the knowledge and expose a narrow API.
