...
- **Incoherent sibling set** — most sibling folders are business concepts but one is `utils/`, `common/`, `services/`, or another catch-all with no bounded contract. Treat this as a medium-strength signal, not a hard rule. A cross-cutting folder such as `lib/`, `shared/`, `schemas/`, `contracts/`, `db/`, or `observability/` may be legitimate when the repo convention makes its contents and dependency direction predictable. Candidate: first ask whether the shared folder has a real contract, then keep it, rename it, decompose it by real concepts, or move files to the concept that owns the knowledge.
